...
void start_menu_render(void) {
	...
	LCD_DrawPicture(GAMEMODE_SELECT_LOGO_X, GAMEMODE_SELECT_LOGO_Y, (const Picture*)&logo);
...
void display_digits(uint32_t* digits_arr, uint32_t digit_arr_size, const Picture*(* numPic_arr), uint32_t start_x, uint32_t y) {
    LCD_DrawFillRectangle(start_x, y, SCOREBOARD_WIDTH, y + numPic_arr[0]->height, BLACK);
    for(int i = digit_arr_size - 1; i >= 0; i--) {
    	const Picture* digit = numPic_arr[digits_arr[i]];
...
void score_display_update(GameState* gs) {
    ...
    // update score
	digits = digits_to_arr(gs->score, digit_arr, SCORE_MAX_DIGITS);
    display_digits(digit_arr, digits, blue_nums, SCORE_START_X, SCORE_Y);


    // update lives
	digits = digits_to_arr(gs->lives_left, digit_arr, SCORE_MAX_DIGITS);
    display_digits(digit_arr, digits, red_nums, LIVES_START_X, LIVES_Y);
